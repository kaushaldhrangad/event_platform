      ...
      <div className="flex min-h-[230px] flex-col gap-3 p-5 md:gap-4  ">
        {!hidePrice && (
          <div className="flex gap-2 ">
            ...
            <p className="p-semibold-14 w-min rounded-full bg-grey-500/10 py-1 px-4 text-grey-500 ">
              {event.category.name}
            </p>
          </div>
        )}

        <p className="p-medium-16 text-grey-500 p-medium-18">
          {formatDateTime(event.startDateTime).dateTime}
        </p>

        <Link href={`/events/${event._id}`}>
          <p className="p-medium-16 text-black md:p-medium-20 line-clamp-2">
            {event.title}
          </p>
        </Link>
